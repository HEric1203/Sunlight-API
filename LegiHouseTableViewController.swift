...
class LegiHouseTableViewController: UITableViewController {
   ...
   // @IBOutlet var senate: UITabBarItem!
    ...
    override func viewDidLoad() {
       ...
       // senate.setTitleTextAttributes([NSFontAttributeName: UIFont(name: "Helvetica", size: 17.0)!], for: UIControlState.normal)
        ...
        Alamofire.request(url, method: .post, parameters: parameters).responseJSON { response in
            ...
            if let resData = swiftjsondata["results"].arrayObject {
                ...
                self.arr = self.arr.filter({($0["chamber"] as! String) == "senate"})
